...
class Header extends React.Component {
  ...
  render() {
    ...
    return (
      <section className="hero is-primary section">
        <div className="hero-body">
          <div id="status" className="container">
            <h1 className="title">Arch Linux Reproducible repository status</h1>
          ...
          {!fetchFailed && suitesStats.map(function(repo, index) {
            return <p key={ index }>[{ repo.name }] repository is { repo.percentage }% reproducible with { repo.bad } bad and { repo.unknown } unknown packages.</p>;
          })}
          </div>
        </div>
      </section>
